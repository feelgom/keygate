...
**Known limitation — stale in-memory secrets (found in real use, not yet fixed):** `GuardState.secrets` is populated once from `load_vault` at `run_foreground_guard` time and never re-read afterward. A `ka set`/`ka remove` made against the vault file while a guard is already live updates the file correctly, but that guard keeps handing out its old in-memory copy for the rest of its session — no error, no staleness warning. Only `ka lock` + `ka unlock` picks up the change. Documented honestly in README's "Security limits" §9 rather than silently left as tribal knowledge; a real fix would need the guard to either re-read the vault per-request (repeated Argon2id cost, by design deliberately expensive) or invalidate/refresh on a `set`/`remove` notification — neither implemented yet.
